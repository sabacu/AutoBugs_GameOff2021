...
func set_car():
	print ("Car 2 : ", GameControl.car_3)
	match GameControl.car_3:
		"Ant":
...
func _on_DetectJump_area_entered(area):
	jumping = true
...
func _on_CheckRace_area_entered(area):
	if area.Check == 1:
		check1 = true
		print(GameControl.car_3," check 1, lap ",race_lap)
	elif area.Check == 2 and check1:
		check2 = true
		print(GameControl.car_3," check 2, lap ",race_lap)
	if area.Check == 3 and check2:
		check3 = true
		print(GameControl.car_3," check 3, lap ",race_lap)
		...
		if race_lap >= 3:
			check4 = true
			print(GameControl.car_3,"fim de corrida")
			$FullStop.wait_time = 1.0
			$FullStop.start()
			if not GameControl.fist_place:
				GameControl.fist_place = true
				GameControl.fist_place_name = str(GameControl.car_3)
			elif not GameControl.second_place:
				GameControl.second_place = true
				GameControl.second_place_name = str(GameControl.car_3)
			elif not GameControl.third_place:
				GameControl.third_place = true
				GameControl.third_place_name = str(GameControl.car_3)
			else:
				GameControl.last_place = true
				GameControl.last_place_name = str(GameControl.car_3)
